...
public class PerfilServicio {
    ...
    public List<Perfil> traerTodoPorNombreOApellido(String nombre, String apellido) {
        return perfilRepositorio.findAllByNombreOrApellidoIgnoreCaseOrderByNombreAsc(nombre, apellido);
    }

    @Transactional(readOnly = true)
    public List<Perfil> traerTodoPorNombreYApellido(String nombre, String apellido) {
        return perfilRepositorio.findAllByNombreAndApellidoIgnoreCaseOrderByNombreAsc(nombre, apellido);
    }

    @Transactional(readOnly = true)
    public List<Perfil> traerTodoPorNombre(String nombre) {
        return perfilRepositorio.findAllByNombreIgnoreCaseOrderByNombreAsc(nombre);
    }

    @Transactional(readOnly = true)
    public List<Perfil> traerTodoPorApellido(String apellido) {
        return perfilRepositorio.findAllByApellidoIgnoreCaseOrderByApellidoAsc(apellido);
    }

    @Transactional(readOnly = true)
    public List<Perfil> traerTodoPorLocalidad(String localidad) {
        return perfilRepositorio.findAllByLocalidadOrderByLocalidadAsc(localidad);
    }

    @Transactional(readOnly = true)
    public List<Perfil> traerTodoPorProvincia(String provincia) {
        return perfilRepositorio.findAllByProvinciaOrderByProvinciaAsc(provincia);
    }
}
